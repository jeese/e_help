...
public class SendSecurityHelpActivity extends Activity implements OnLongClickListener{

	private static final String[] sen={"常用语句", "火灾了","地震了","被盗了","触电了","煤气泄漏","发生爆炸了"};
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		lo=new Location(SendSecurityHelpActivity.this);
	    ...
	    btn_video.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				Intent intent=new Intent(SendSecurityHelpActivity.this,Video.class);  
	...
	private class Send extends AsyncTask<Void, Void, String> {
        ...
        protected void onPostExecute(String result) {   	
        	...
        	if(result.equals("network error")){
        		Toast.makeText(SendSecurityHelpActivity.this,"您还没有联网", Toast.LENGTH_SHORT).show();
            	//pro.setVisibility(View.INVISIBLE);
        	}
        	if(result.equals("error")){
        		Toast.makeText(SendSecurityHelpActivity.this,"连接服务器失败", Toast.LENGTH_SHORT).show();
            	//pro.setVisibility(View.INVISIBLE);
        	}
            try {
            	switch (new JSONObject(result).getInt("state")) {
            	case 1:
            		Toast.makeText(SendSecurityHelpActivity.this, "发送成功", Toast.LENGTH_SHORT).show();
            		ControlActivity.SetPager(1);
            		finish();
            		break;
            	default:
            		Toast.makeText(SendSecurityHelpActivity.this, "发送失败", Toast.LENGTH_SHORT).show();
            	}
				     ...
				     case MotionEvent.ACTION_UP:
				    	 ...
				    	 try{
				    		 System.out.print("******进入try");
								...
								Toast.makeText(SendSecurityHelpActivity.this, "录音成功" + audio_file, Toast.LENGTH_SHORT).show();
							}catch(Exception e) {
